...
export const PYTH_CONTRACTS: Record<number, `0x${string}` | undefined> = {
  ...
  // Base Sepolia (84532): set via env for safety to avoid hardcoding a wrong address
  84532: (process.env.NEXT_PUBLIC_PYTH_BASE_SEPOLIA as `0x${string}`) || undefined,
}

export function getPythAddress(chainId: number): `0x${string}` {
  const addr = PYTH_CONTRACTS[chainId]
  if (!addr) throw new Error(`Pyth contract address not configured for chain ${chainId}. Set NEXT_PUBLIC_PYTH_BASE_SEPOLIA`)
  return addr
}
